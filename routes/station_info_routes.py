...
def station_info():
    ...
    if request.method == 'POST':
        if 'station_query' in request.form:
            ...
            for line, data in metro_lines.items():
                if line != "Interchanges" and query in data["stations"]:
                    ...
                    nearby_stations = (
                        data["stations"][max(0, idx - 2):idx] +
                        data["stations"][idx + 1:idx + 3]
                    )

        elif 'line_query' in request.form:
            selected_line = request.form['line_query']
