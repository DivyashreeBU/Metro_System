...
def book_ticket():
    if request.method == 'POST':
        ...
        if not to_station:
            flash("Missing value or From and To stations cannot be the same")
            return redirect('/book')

        raw_fare = round(random.uniform(10, 50), 2)
        fare = math.ceil(raw_fare)
        ...
        # ✅ Generate QR code after ticket is saved
        import qrcode
        import os
        ...
        # ✅ Save QR path to ticket and commit again
        ticket.qr_code_path = qr_path
        db.session.commit()

        print("QR Code Path:", ticket.qr_code_path)
...
@ticket_bp.route('/search-ticket', methods=['GET', 'POST'])
def search_ticket():
    if request.method == 'POST':
        ...
        if ticket:
            ...
            flash("❌ No matching ticket found.")
            return redirect('/search-ticket')

    return render_template('search_ticket.html', stations=all_stations)
